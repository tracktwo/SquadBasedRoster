...
function Launch-Make([string] $makeCmd, [string] $makeFlags, [string] $sdkPath, [string] $modSrcRoot) {
    ...
    Write-Host "Development dir is $developmentDirectory"
    Write-Host "MOdroot is $modSrcRoot"
    ...
    $outAction = {
        ...
        if (($outTxt -Match "Error|Warning") -And ($outTxt -Match $messagePattern)) {
            ...
            # Test if this file actually exists or not, (e.g. is it in our mod root, or is it a base game diagnostic?)
            if ($replacementTxt -Match "^(.*)\(\d+\) :") {
...
if ($forceFullBuild) {
    ...
    Write-Host "Skipping clean because -forceFullBuild not passed"
}

# copy source to staging
Robocopy.exe "$modSrcRoot" "$sdkPath\XComGame\Mods\$modNameCanonical" *.* /S /E /DCOPY:DA /COPY:DAT /PURGE /MIR /NP /R:1 /W:30 /XO
...
Robocopy.exe "$sdkPath/Development/SrcOrig" "$sdkPath/Development/Src" *.uc *.uci /S /E /DCOPY:DA /COPY:DAT /PURGE /MIR /NP /R:1000000 /W:30 /XO
Write-Host "Mirrored."
